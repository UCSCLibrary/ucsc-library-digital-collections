...
#  end
  ...
  describe "#sparql" do
    ...
    context "using a two subject terms" do
      subject { authority.sparql('search term').gsub(/[\s\n]+/,"") }
      it {
        is_expected.to include('(regex(?name, "search", "i") || regex(?alt, "search", "i"))'.gsub(/[\s\n]+/,""))
        is_expected.to include('(regex(?name, "term","i") || regex(?alt, "term","i") )'.gsub(/[\s\n]+/,""))
        is_expected.to include('ORDER BY ?name'.gsub(/[\s\n]+/,""))
      }
    end
  end
end
